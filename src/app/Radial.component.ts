...
@Component({
  selector: 'radial',
  template: `<div #myDiagramDiv id="myDiagramDiv" style="border: solid 1px black; background: white; width: 100%; height: 600px"></div>`
})

export class RadialComponent implements AfterViewInit {

@ViewChild('myDiagramDiv') element: ElementRef;
nodeClicked(e, root) {
  let diagram = root.diagram;
...
ngAfterViewInit() {
  ...
  myDiagram =
    $(go.Diagram, this.element.nativeElement, // must be the ID or reference to div
      {
  ...
  let someone = nodeDataArray[Math.floor(Math.random() * nodeDataArray.length)];
  this.nodeClicked(null, myDiagram.findNodeForData(someone));
  // called when "Set Max Layers" button is clicked
  /*adjustMaxLayers() {
  let newMaxLayers = (document.getElementById('maxLayersChanger') as any).value;
  function IsNumeric(val) {
    return Number(parseInt(val, 10)) === val;
  }
  if (!IsNumeric(newMaxLayers) || newMaxLayers < 1 || newMaxLayers > 10) {
    ...
    let lay = myDiagram.layout as RadialLayout;
    lay.maxLayers = Math.max(1, Math.min(newMaxLayers, 10));
    this.nodeClicked(null, lay.root);
  }*/
}
}
